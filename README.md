...
| **4 — Subagents** | `agents/` | When invoked by a playbook | Subagent definitions deployed to `.devin/agents/` (Devin) and `.claude/agents/` (Claude Code) |
...
agents/                                 Tier 4 — subagent definitions (→ .devin/agents/ for Devin, .claude/agents/ for Claude)
  investigation-reviewer/AGENT.md       Reviews investigation conclusions before writing (copy of agent-orchestrator source)
  postmortem-writer/AGENT.md            Generates HTML post-mortem from structured findings
...
playbooks/                              Tier 2 — on demand (→ target .context/playbooks/)
  ...
  investigate/                          Incident investigation and post-mortem production (1)
    postmortem.md
```
